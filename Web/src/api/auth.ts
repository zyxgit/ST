...
export function login(data: LoginCommand) {
  return request.post<LoginResult>('/identity/api/user/login', data)
}

export function logout(data: RefreshTokenCommand) {
  return request.post<void>('/identity/api/user/logout', data)
}

export function getCurrentUser() {
  return request.get<CurrentUser>('/identity/api/user/me')
}
