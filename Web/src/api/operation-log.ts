...
export function getOperationLogs(params: OperationLogQuery) {
  return request.get<PagedResult<OperationLogListItem>>('/operationlog/operation-logs', { params })
}

export function getOperationLogDetail(id: number) {
  return request.get<OperationLogDetail>(`/operationlog/operation-logs/${id}`)
}
