...
function getRemainingMs(createTime: string): number {
  const expireAt = new Date(createTime).getTime() + TIMEOUT_MINUTES * 60 * 1000
  return expireAt - now.value
}
...
const columns: DataTableColumns<OrderDto> = [
  ...
  },
  {
    ...
    render: (row: OrderDto) => {
      ...
      const remaining = getRemainingMs(row.createTime)
      const text = formatRemaining(remaining)
      const type = remaining <= 0 ? 'error' : remaining <= 60_000 ? 'warning' : 'default' as const
